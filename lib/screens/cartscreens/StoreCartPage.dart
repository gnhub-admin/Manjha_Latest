...
class _StoreCartPageState extends State<StoreCartPage> {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        Navigator.of(context).pop(cartChanged);
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(
            "${translate("My Cart")}" +
                (weight > 0
                    ? ' (${weight}${translate("KG")})'
                    : ''), //+ (Common.cartCount > 0 ? " (${Common.cartCount})" : ""),
        ...
        body: _loading == true
            ...
            : cartList.length <= 0
                ? Common.containerNoDataFound(
                    '${translate('Cart is empty.')}') //Center(child: Icon(Icons.hourglass_empty_rounded))
                : Column(
                    children: [
                      Expanded(
                              ...
                              children: [
                                Column(
                                  children: [
                                    ...
                                    //           title: Text(
                                    //             postcodeMessage,
                                    //             style: TextStyle(color: postcodeDeliverable ? Colors.green : Colors.red),
                                    //           ),
                                    //           trailing: IconButton(
                                    //             icon: Icon(Icons.cancel),
                                    //             onPressed: () {
                                    //               setState(() {
                                    //                 postcodeApplied = !postcodeApplied;
                                    //                 if (!postcodeApplied) postcodeDeliverable = true;
                                    //               });
                                    //               // EasyLoading.showToast('Removed');
                                    //             },
                                    //           ),
                                    ...
                                    //           ),
                                    //           subtitle: Text("Check order is deliverable on your pincode!!".tr),
                                    //           trailing: IconButton(
                                    //             icon: Icon(
                                    //               Icons.check_circle,
                                    //               color: Colors.grey,
                        ...
                        child: MaterialButton(
                            ...
                            onPressed: isWarning()
                                ...
                                : !postcodeDeliverable
                                    ? null
                                    : () async {
                                        String custId = saveUser()
                                                ?.data
                                                ?.id
                                                .toString() ??
                                            ""; //await LoginModel.getCustomerId();
                                        print(custId);
                                        if (custId.isEmpty) {
                                          // Common.pushPage(context, LoginScreen());

                                          return;
                                        } else {
                                          // Common.pushPage(context, CheckoutPage());
                                        }
                                        ...
                                        // this.initPlatformState(total);
                                        MixpanelController.logScreen(
                                          MixpanelController.PageCheckout,
                                        );
                                          ...
                                          MaterialPageRoute(
                                            builder: (BuildContext context) {
                                              return StoreCheckoutPage(
                                                  // onPagePop: () {
                                                  //   prductlist.productApiCall(categoryid: widget.categoryid, index: 0, brandid: widget.brandid);
                                                  // }
                                                  );
                                              // return StoreDeliveryPage();
                                            },
                                          ),
                                        );
                                      },
        ...
        //           offset: Offset(0, -2), // Offset for upper side elevation
        //           blurRadius: 8,
        //           spreadRadius: 0,
        //         ),
        //       ],
        ...
        //       ? Center(
        //           child:
        //               CircularProgressIndicator(strokeWidth: 2, color: kheader),
        //         )
        //       : Padding(
        //           padding:
...
class _StoreCartPageState extends State<StoreCartPage> {
  ...
  void couponUpdate() async {
    ...
    final response = await http.post(Common.getURL("store_couponCheck"),
        ...
        body: jsonEncode(<String, String>{'coupon_code': _couponController.text}));
    EasyLoading.dismiss();
    print(response.body);
    if (response.statusCode == 200) {
      final Map<String, dynamic> resBody = jsonDecode(response.body);
  ...
  void postcodeUpdate() async {
    ...
    final response = await http.post(Common.getURL("store_postcodeCheck"),
        ...
        body: jsonEncode(<String, String>{'postcode': _pincodeController.text}));
    EasyLoading.dismiss();
    print(response.body);
    if (response.statusCode == 200) {
      final Map<String, dynamic> resBody = jsonDecode(response.body);
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        body: _loading == true
            ? Center(child: CircularProgressIndicator(color: kheader, strokeWidth: 2))
            : cartList.length <= 0
                ? Common.containerNoDataFound(
                    '${translate('Cart is empty.')}') //Center(child: Icon(Icons.hourglass_empty_rounded))
                : ScrollConfiguration(
                              ...
                              Visibility(
                                ...
                                child: Container(
                                    ...
                                    child: Card(
                                        color: Colors.red[200],
                                        child: Padding(
                                            padding: EdgeInsets.all(12), child: BoldText(warning, 16.0, kblack)))),
                              ),
                              ListView.builder(
                                  shrinkWrap: true,
                                  physics: NeverScrollableScrollPhysics(),
                                            ...
                                            child: Column(
                                              children: [
                                                Row(
                                                  children: [
                                                    Container(
                                                      constraints: BoxConstraints.tightFor(width: 110.0, height: 135.0),
                                                      decoration: BoxDecoration(
                                                        color: kheader.withOpacity(0.1),
                                                      ...
                                                      child: Padding(
                                                        padding: const EdgeInsets.all(8.0),
                                                        child: Image.network(cartList[index].getImageUrl(),
                                                            fit: BoxFit.cover),
                                                      ),
                                                    ),
                                                    Expanded(
                                                      child: Padding(
                                                        ...
                                                        child: Column(
                                                          ...
                                                          children: [
                                                            ...
                                                            Row(
                                                              children: [
                                                                getQuantityContainer(cartList[index]),
                                                                Expanded(child: SizedBox()),
                                                                Text(
                                                                  "${translate("Rs")}." +
                                                                      cartList[index].getTotal().toString(),
                                                                  style: TextStyle(
                                            ...
                                            children: [
                                              Container(
                                                  ...
                                                  child: Text(
                                                    totalList[index]['title'],
                                                    style: TextStyle(
                                                        fontSize: 16, color: kgreyDark, fontWeight: FontWeight.w400),
                                                  )),
                                              Spacer(),
                                              Container(
                                                padding: EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                                                child: Text('${translate("Rs")}.' + totalList[index]['text'].toString(),
                                                    style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                                              ),
                                            ],
                                          ),
                                          index + 1 == totalList.length ? SizedBox() : Divider()
                                        ],
                                      );
                                    },
                                  ),
                                ...
                                child: couponApplied
                                  ...
                                  shape: RoundedRectangleBorder(
                                    borderRadius: BorderRadius.circular(12.0),
                                        ...
                                        trailing: IconButton(
                                          icon: Icon(Icons.cancel, color: kheader),
                                          onPressed: () {
                                            _couponController.text = '';
                                            couponUpdate();
                                            // setState(() {
                                  ...
                                  shape: RoundedRectangleBorder(
                                    borderRadius: BorderRadius.circular(12.0),
                                        ...
                                        trailing: IconButton(
                                          icon: Icon(Icons.arrow_forward_ios_rounded, size: 20, color: Colors.grey),
                                          onPressed: () {
                                            _showCouponDialog();
                                            // couponUpdate();
        ...
        bottomNavigationBar: Container(
          ...
          child: _loading == true
              ? Center(
                  child: CircularProgressIndicator(strokeWidth: 2, color: kheader),
                )
              : Padding(
                        ...
                        child: MaterialButton(
                            ...
                            onPressed: isWarning()
                                ...
                                : !postcodeDeliverable
                                    ...
                                    : () async {
                                        String custId =
                                            Session.getCustId().toString(); //await LoginModel.getCustomerId();
                                        print(custId);
                                        if (custId.isEmpty) {
                                          // Common.pushPage(context, LoginScreen());
        ...
        //                 child: Text(
        //                     getTotal(), //cartObj.totals[cartObj.totals.length - 1].text
        //                     style: TextStyle(
        ...
        //                                     .toString(); //await LoginModel.getCustomerId();
        //                                 print(custId);
  ...
  Widget _incrementButton(CartModel cartItem) {
    ...
    return MaterialButton(
      ...
      visualDensity: VisualDensity.compact,
      elevation: 0,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6), side: BorderSide(color: kColorButtonCart)),
      onPressed: () async {
        // setState(() {
        //   cartItem.quantity += cartItem.getQtyLot();
        //   // item.quantity++;
        ...
        _quantityController.text = cartItem.quantity.toString();
        bool result = await showQuanityBox(context, false);
        if (result == '' || result == false || _quantityController.text.isEmpty) {
          _quantityController.text = '';
          print('Quantity dialog cancelled...');
  ...
  Widget getOptionText(List<dynamic> option) {
    return Text(option.map((e) => e['value']).join(','), style: TextStyle(color: kColorButtonCart));
  }
  ...
  _fetchCart() async {
    EasyLoading.show();
    warning = '';
    final response = await http.get(Common.getURL("store_cart"), headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
      'Cookie': Common.getCookie().toString()
      // "content-type": "application/x-www-form-urlencoded",
    });
      ...
      builder: (context) => AlertDialog(
        ...
        actions: <Widget>[
          TextButton(
              style: ButtonStyle(
                  backgroundColor: MaterialStateProperty.resolveWith((states) => themecolor.withOpacity(0.5))),
              child: Text('Apply'.tr, style: TextStyle(color: kColorButtonCart)),
              onPressed: () {
                if (_couponController.text.length < 3) {
                  EasyLoading.showToast("Please enter min 3 character.");
                  return;
        ...
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            TextButton.icon(
              ...
              icon: Icon(Icons.add_shopping_cart, color: kColorButtonCart),
              label:
                  BoldText((Lang.get(addNew ? "${translate('Add')}" : "${translate('Update')}")), 14, kColorButtonCart),
              onPressed: () {
                if (_quantityController.text.isEmpty) {
                  EasyLoading.showToast("Please enter quantity.");
                  return;
                ...
                if (int.tryParse(_quantityController.text) != null &&
                    (int.tryParse(_quantityController.text)! <= 0 || int.tryParse(_quantityController.text)! > 10000)) {
                  EasyLoading.showToast("Please enter proper quantity.");
                  return;
                }
                // Close the dialog
